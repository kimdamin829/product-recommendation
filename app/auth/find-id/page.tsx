...
export default function FindIdPage() {
  ...
  const verifyAndFind = async () => {
    ...
    try {
      ...
      setMaskedUsername(findData.maskedUsername || '')
      setStep(3)
    } catch (err: any) {
          ...
          {step === 3 && (
            <div className="text-center">
              <div className="space-y-1">
                ...
                {maskedUsername ? (
                  <p className="text-lg font-semibold text-gray-900 mt-2">아이디: {maskedUsername}</p>
                ) : null}
                <p className="text-sm text-gray-500 mt-2">(테스트 환경: 문자는 발송되지 않습니다.)</p>
              </div>
